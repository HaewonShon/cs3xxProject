...
Purpose: Fragment shader for Blinn Shading implementation.
...
vec3 CalcDirectionalLight(vec3 lightDir, vec3 viewDir, int lightIndex, vec3 worldNormal)
{
   ...
   // specular
   vec3 reflect = normalize(lightDir + viewDir);
   float spec = pow(max(dot(worldNormal, reflect), 0.0), 32);
...
vec3 CalcPointLight(vec3 viewDir, int lightIndex, vec3 worldNormal, vec3 worldPos)
{
   ...
   // diffuse
   vec3 diffuse = max(dot(lightDir, worldNormal), 0.0) * lights[lightIndex].diffuse * texture(diffuseTexture, textureUV).xyz;
   // specular
   vec3 reflect = normalize(lightDir + viewDir);
   float spec = pow(max(dot(worldNormal, reflect), 0.0), 32);
   vec3 specular = spec * lights[lightIndex].specular * texture(specularTexture, textureUV).xyz;
